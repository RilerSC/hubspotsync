...
ALTER PROCEDURE [dbo].[sp_hubspot_insert]
AS
BEGIN 
    SET NOCOUNT ON;
    ...
    IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[insert_contacts_hs]') AND type in (N'U'))
    BEGIN
        PRINT '📋 Creando tabla insert_contacts_hs...'
        
        CREATE TABLE [dbo].[insert_contacts_hs] (
            ...
            CONSTRAINT [PK_hb_contacts_sync] PRIMARY KEY CLUSTERED ([sync_id] ASC),
            CONSTRAINT [IX_hb_contacts_sync_cedula] UNIQUE NONCLUSTERED ([no__de_cedula] ASC)
        );
        
        PRINT '✅ Tabla hb_contacts_sync creada exitosamente con todos los campos.'
    END
    ELSE
    BEGIN
        PRINT '📋 Tabla hb_contacts_sync ya existe - procediendo con actualización.'
    END
    ...
    DELETE FROM [dbo].[insert_contacts_hs];
...
[Encargado] as (
    ...
    INSERT INTO [dbo].[insert_contacts_hs] (
        [numero_asociado],
        [no__de_cedula], 
        [firstname],
        [lastname],
    ...
    SELECT DISTINCT
        ...
        REPLACE(REPLACE(REPLACE(REPLACE(t1.ccedulasoc, '-', ''), ' ', ''), '.', ''), ',', '') AS [no__de_cedula],  -- 🔑 Campo clave para HubSpot (solo números)
        ...
        CASE t33.creditos WHEN 'si' THEN 'si' ELSE 'no' END AS [refuncion_ii],
    ...
    WHERE t1.ccedulasoc IS NOT NULL 
      AND t1.ccedulasoc != ''  -- 🔑 Solo cédulas válidas (solo números) para HubSpot
      and t1.ccondasoci = '01'  -- Solo activos
    ...
    PRINT ''
    PRINT '🚀 Tabla hb_contacts_sync lista para sincronización con HubSpot'
