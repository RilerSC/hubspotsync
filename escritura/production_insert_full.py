...
def production_insert_full():
    ...
    with open(conflict_report_file, 'w', newline='', encoding='utf-8') as conflict_file, \
        ...
        # SEGURIDAD: Importar funciones de validación
        ...
        for i, contact_data in enumerate(insert_data, 1):
            ...
            if not isinstance(contact_data, dict):
                logger.warning(f"Registro {i} no es un diccionario válido, omitiendo")
                stats['invalid'] += 1
                continue

            # SEGURIDAD: Sanitizar todos los campos antes de usar
            cedula_raw = contact_data.get('no__de_cedula', 'N/A')
            cedula = sanitize_string(str(cedula_raw), max_length=20) if cedula_raw else 'N/A'
            email = sanitize_string(contact_data.get('email', 'N/A'), max_length=255)
            firstname = sanitize_string(contact_data.get('firstname', 'N/A'), max_length=100)
            lastname = sanitize_string(contact_data.get('lastname', 'N/A'), max_length=100)
            numero_asociado = sanitize_string(str(contact_data.get('numero_asociado', 'N/A')), max_length=50)
            ...
            try:
                ...
                # 1. SEGURIDAD: Validar formato de cédula
                if not cedula or cedula == 'N/A' or not validate_cedula(cedula):
                    ...
                    logger.warning(f"Registro {i}: Cédula inválida o faltante: {cedula_masked}")
                    continue
